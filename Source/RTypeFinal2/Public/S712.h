...
#include "S712.generated.h"

class AActor;
class UPrimitiveComponent;

UCLASS(Blueprintable)
class AS712 : public AEnemyActor {
    GENERATED_BODY()
public:
    UPROPERTY(BlueprintReadWrite, EditAnywhere, meta=(AllowPrivateAccess=true))
    AEnemyActor* ParentEnemy;
    
    AS712(const FObjectInitializer& ObjectInitializer);

    UFUNCTION(BlueprintCallable)
    void ShotOnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult);
